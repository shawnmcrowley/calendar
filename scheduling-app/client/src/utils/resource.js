...
export async function handleRegister (email, username, password, navigate) {
    try {
        const request = await fetch("http://mercury:4000/register", {
            method: "POST",
            body: JSON.stringify({
                email,
                username,
...
export async function handleLogin (username, password, navigate) {
    try {
        const request = await fetch("http://mercury:4000/login", {
            method: "POST",
            body: JSON.stringify({
                username,
                password,
...
export async function handleCreateSchedule(
    ...
    try {
        await fetch("http://mercury:4000/schedule/create", {
            method: "POST",
            body: JSON.stringify({
...
export function fetchBookingDetails (
    ...
    fetch(`http://mercury:4000/schedules/${user}`, {
        method: "POST",
        body: JSON.stringify({
            username: user,
        }),
